...
flowchart LR
    ...
    UI --> CLIENT
    UI --> WS
    CLIENT -->|"Bearer JWT, 401 自動 refresh"| REST
    WS -->|"token=JWT 串流訊息"| SOCK
    WORLD --> BSEAM
...
> - api client 用攔截器統一加上 JWT，遇 401 會自動 refresh 並重試。
...
```mermaid
flowchart TD
    A["使用者填前測問卷"] --> B["計算 stance score 1-7 + 開放題 embedding 384d"]
    B --> C[("UserStanceProfile: stance_score, q9_embedding")]

    C --> D{"立場分類"}
    D -->|支持或反對| E["進配對佇列 MatchQueueEntry"]
    D -->|中立| F["建立 AI 對話 DialogueSessionRecord"]

    E --> G[("DialogueMatch: 兩人 + 距離指標 + room_id")]
    G --> H[("MatchMessage: content, embedding")]
    F --> I[("AIConversation: prompt/response, embedding")]

    H --> J["即時分析 emotion/drift/stalemate"]
    I --> J
    J --> K[("MatchStanceDrift: drift 時間序列")]

    H --> L["建 CCND 語意樹"]
    I --> L
    L --> M[("semantic_tree_state 存於 Match.stats / SessionRecord")]

    G --> N["對話結束 on_commit"]
    N --> O["M6 觀點萃取 pipeline: 品質過濾 + pgvector 去重"]
    O --> P[("ViewpointNode 知識庫 人工審核")]

    A2["使用者填後測問卷"] --> Q[("PostDialogueResponse: s_pre, s_post, delta_s, stance_centrism")]
```

重點：立場向量、每則發言向量、觀點向量全是同一 384 維空間、同存 pgvector，讓「配對距離」「立場漂移」「觀點去重」都能用 SQL 端的 CosineDistance 直接算。
...
flowchart TD
    subgraph 一_立場量測與向量化
        A["前測 量表 Q1-Q8 + 開放題 Q9"] --> B["stance score 1-7 反向題校正後平均"]
        A --> Demb["Sentence-Transformers MiniLM 384 維"]
        Demb --> E[("pgvector q9_embedding")]
    end

    subgraph 二_異質配對最大化距離
        B --> Gm["match score = 0.6 量表距離 + 0.4 語意餘弦距離"]
        E --> Gm
        Gm --> H{"對立立場閘門 找最相反的人"}
        H -->|配對成功| I["H-H 對話室"]
        H -->|中立或無對手| R
    ...
    subgraph 三_RAG_AI代理人對立立場
        R["AI 對話"] --> J["Chroma 檢索 top-k"]
        KB[("ChromaDB 知識庫 離線建置")] --> J
        J --> K["脈絡 + 歷史 + 對話階段"]
        K --> L["Claude 串流生成 system prompt 快取"]
        L --> M["逐句串流回使用者"]
    end
    ...
    subgraph 四_即時NLP監測
        ...
        N --> O["離題 餘弦 vs 議題錨點"]
        N --> P["僵局 距離標準差"]
        N --> Qd["立場漂移 發言均值 vs 基準"]
    end

    subgraph 五_CCND語意樹
        I --> T{"依議題分派"}
        M --> T
        T -->|一般| Uo["OpenAI 結構化 JSON 固定 6 錨點"]
        T -->|核能| V["本地兩階段 BERT macro→micro"]
        Uo --> W["每人一棵推理樹 可時間軸回放"]
        V --> W
    end

    subgraph 六_結算與知識庫
        W --> X["CCND 快照分析 Jaccard 相似度 + 新穎度"]
        A2["後測問卷"] --> Y["delta_s + stance_centrism 去極化指標"]
        W --> Z["M6 觀點萃取 → pgvector 去重"]
        Z --> KB2[("ViewpointNode 知識庫")]
    end
```

設計理由（為何本地 embedding、pgvector 與 ChromaDB 分工、為何「最大化距離」）見 [TECHNICAL_OVERVIEW.md](./TECHNICAL_OVERVIEW.md)。
...
flowchart TD
    ...
    HHChat --> Tree
    Tree --> Post["填後測問卷"]
    Post --> Result["結算 立場軌跡 + 思辨評級 可匯出長圖"]
    Result --> KB["逛觀點知識庫"]

    Home --> Lobby["進 Godot 虛擬大廳"]
    Lobby --> Meet["遇到玩家 張貼議題 表情互動 語音"]
    Meet --> Match["就地配對進對話"]
    Match --> HHChat
...
> 前端頁面呈現、CCND 視覺化、結算畫面、成就、Godot 大廳互動為我負責的部分；分流、配對、AI 生成、結算數值計算由後端負責。
